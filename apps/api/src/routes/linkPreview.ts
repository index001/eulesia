...
router.get('/link-preview', async (req: Request, res: Response) => {
  try {
    ...
    if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
      return res.status(400).json({ success: false, error: 'Only HTTP/HTTPS URLs supported' })
    }
    ...
    try {
      ...
      return res.status(404).json({ success: false, error: 'Failed to fetch URL' })
    }
    clearTimeout(timeout)
    ...
    if (!contentType.includes('text/html') && !contentType.includes('application/xhtml')) {
      await upsertPreview(url, { error: true })
      return res.status(404).json({ success: false, error: 'Not an HTML page' })
    }
    ...
    if (!reader) {
      await upsertPreview(url, { error: true })
      return res.status(404).json({ success: false, error: 'No response body' })
    }
    ...
    if (!metadata.title) {
      await upsertPreview(url, { error: true })
      return res.status(404).json({ success: false, error: 'No metadata found' })
    }
...
async function upsertPreview(url: string, data: {
  title?: string | null
