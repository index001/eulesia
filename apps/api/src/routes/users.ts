...
router.get('/me/data', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user!.id

  // Get all user data
  const { threads, comments, clubMembers, messages, notifications, userSubscriptions } = await import('../db/index.js')
