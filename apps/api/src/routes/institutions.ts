...
function validateBusinessId(id: string, country: string): boolean {
  switch (country) {
    case 'FI': return validateFinnishBusinessId(id)
    // Other countries: basic format validation
    case 'SE': return /^\d{6}-\d{4}$/.test(id) // Swedish org.nr
    case 'EE': return /^\d{8}$/.test(id)       // Estonian registry code
    case 'DE': return /^(DE)?\d{9}$/.test(id)  // German USt-IdNr
    default: return id.length >= 5 && id.length <= 30
  }
}

// POST /institutions/create — Create a new organization account
router.post('/create', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const data = z.object({
    ...
    institutionType: z.enum(['organization', 'agency']).default('organization')
  }).parse(req.body)
  ...
  if (data.businessId && data.businessIdCountry) {
    if (!validateBusinessId(data.businessId, data.businessIdCountry)) {
      res.status(400).json({ success: false, error: 'Invalid business ID format' })
      return
    }
    ...
    if (existing) {
      res.status(400).json({ success: false, error: 'Business ID is already registered' })
      return
    }
  }
...
router.get('/available', authMiddleware, asyncHandler(async (_req: AuthenticatedRequest, res: Response) => {
...
router.post('/:institutionId/claim', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user!.id
  ...
  const [existing] = await db
    .select({ id: institutionManagers.id, status: institutionManagers.status })
    .from(institutionManagers)
    .where(and(
  ...
  if (existing) {
    res.status(400).json({ success: false, error: 'You already have a claim for this institution' })
    return
  }
...
router.get('/:institutionId/check', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user!.id
  const { institutionId } = req.params
  ...
  res.json({ success: true, data: { canManage: !!manager, role: manager?.role || null } })
}))

// Admin: GET /institutions/claims — List pending claims
router.get('/claims', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
...
router.patch('/claims/:claimId', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  if (req.user!.role !== 'admin') {
    res.status(403).json({ success: false, error: 'Admin access required' })
    return
  }
  ...
  if (claim.status !== 'pending') {
    res.status(400).json({ success: false, error: 'Claim has already been processed' })
    return
  }
