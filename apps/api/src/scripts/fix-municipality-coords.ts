...
import { searchNominatim, extractBounds, extractPopulation } from '../services/nominatim.js'
...
async function fixMunicipalityCoords() {
  ...
  for (const muni of missing) {
    try {
      ...
      console.error(`  ERROR: ${muni.name}:`, error instanceof Error ? error.message : error)
      failed++
    }
  }
