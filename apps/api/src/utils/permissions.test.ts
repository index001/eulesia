...
const userContent = { authorId: 'user-1', source: 'user' as const, aiGenerated: false }
const botContent = { authorId: 'bot-1', source: 'minutes_import' as const, aiGenerated: false }
const aiContent = { authorId: 'bot-1', source: null, aiGenerated: true }
const otherContent = { authorId: 'user-2', source: 'user' as const, aiGenerated: false }
...
describe('isBotContent', () => {
  ...
  it('returns false for null/undefined source without aiGenerated', () => {
    expect(isBotContent({ authorId: 'x', source: null, aiGenerated: false })).toBe(false)
    expect(isBotContent({ authorId: 'x', source: null, aiGenerated: null })).toBe(false)
  })
})
