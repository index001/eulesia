 ...
 */
export async function fetchSession(sessionId: number): Promise<VnSession | null> {
  ...
  if (html.includes('Istuntoa ei löydy') || html.includes('Sivua ei löytynyt')) {
    return null
  }

  // Extract session title — typically in a heading like:
  // "Valtioneuvoston yleisistunto 5.2.2026 VN 7/2026"
  const titleMatch = html.match(/(Valtioneuvoston yleisistunto|Raha-asiainvaliokunta|Tasavallan presidentin esittely)\s+(\d{1,2}\.\d{1,2}\.\d{4})\s*((?:VN|RV)\s+\d+\/\d{4})?/i)
 ...
 */
export async function fetchDecision(decisionId: number, ministry?: string): Promise<VnDecision | null> {
  ...
  if (html.includes('Päätöstä ei löydy') || html.includes('Sivua ei löytynyt')) {
    return null
  }
  ...
  // Extract minister
  const ministerMatch = html.match(/(?:Ministeri|Minister)[^<]*<[^>]*>([^<]+)/i)
    || html.match(/(?:Esittelevä ministeri|Ministeri)\s*[:.]?\s*([^\n<]+)/i)
  const minister = ministerMatch ? stripHtml(ministerMatch[1]).trim() : undefined

  // Extract presenter
  const presenterMatch = html.match(/(?:Esittelijä|Esittelevä)[^<]*<[^>]*>([^<]+)/i)
    || html.match(/(?:Esittelijä)\s*[:.]?\s*([^\n<]+)/i)
  const presenter = presenterMatch ? stripHtml(presenterMatch[1]).trim() : undefined

  // Extract session info
  const sessionMatch = html.match(/(Valtioneuvoston yleisistunto|Raha-asiainvaliokunta|Tasavallan presidentin esittely)\s+(\d{1,2}\.\d{1,2}\.\d{4})/i)
  ...
  const asiaMatch = html.match(/(?:>Asia\s*<|<strong>Asia<\/strong>)([\s\S]*?)(?=(?:>Esitys\s*<|<strong>Esitys|>Päätös\s*<|<strong>Päätös|$))/i)
  if (asiaMatch) {
    const text = stripHtml(asiaMatch[1])
    if (text.length > 10) contentParts.push(`Asia:\n${text}`)
  }

  const esitysMatch = html.match(/(?:>Esitys\s*<|<strong>Esitys<\/strong>)([\s\S]*?)(?=(?:>Päätös\s*<|<strong>Päätös|$))/i)
  if (esitysMatch) {
    const text = stripHtml(esitysMatch[1])
    if (text.length > 10) contentParts.push(`Esitys:\n${text}`)
  }

  const paatosMatch = html.match(/(?:>Päätös\s*<|<strong>Päätös<\/strong>)([\s\S]*?)(?=(?:<\/div>|<\/article>|<footer|<nav|$))/i)
  if (paatosMatch) {
  ...
  if (contentParts.length === 0) {
    const mainMatch = html.match(/<article[^>]*>([\s\S]*?)<\/article>/i)
      || html.match(/<div[^>]*class="[^"]*journal-content[^"]*"[^>]*>([\s\S]*?)<\/div>/i)
...
export async function findLatestSessionId(startFrom = 400): Promise<number> {
  ...
  for (let id = startFrom; id > Math.max(startFrom - 30, 1); id--) {
    ...
    try {
      ...
      const html = await response.text()
      if (!html.includes('Istuntoa ei löydy') && !html.includes('Sivua ei löytynyt') && html.includes('decisionId=')) {
 ...
 */
export async function fetchRecentSessions(count = 5, startFromId?: number): Promise<VnSession[]> {
  const latestId = startFromId ?? await findLatestSessionId()
  const sessions: VnSession[] = []
  let misses = 0

  for (let id = latestId; id > 0 && sessions.length < count && misses < 15; id--) {
