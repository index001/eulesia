...
export function mapNominatimToLocationType(result: NominatimResult): string {
  ...
  if (type === 'state' || type === 'region' || type === 'province' || place_rank <= 8) {
    return 'region'
  }
 ...
 */
export function mapNominatimToAdminLevel(result: NominatimResult): number | null {
  ...
  // Region level
  if (type === 'state' || type === 'region' || type === 'province' || (place_rank > 4 && place_rank <= 8)) {
    return 4
  }
  ...
  // Municipality level
  if (type === 'city' || type === 'town' || type === 'municipality' || (place_rank > 12 && place_rank <= 16)) {
    return 7
  }
