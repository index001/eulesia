...
export async function runGdprCleanup(): Promise<CleanupResult> {
  ...
  try {
    ...
    result.errors.push(`Sessions cleanup: ${err instanceof Error ? err.message : String(err)}`)
  }
  ...
  try {
    ...
    result.errors.push(`Magic links cleanup: ${err instanceof Error ? err.message : String(err)}`)
  }
  ...
  try {
    ...
    result.errors.push(`Invite codes cleanup: ${err instanceof Error ? err.message : String(err)}`)
  }
