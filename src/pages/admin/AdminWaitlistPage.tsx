...
import { useAdminWaitlist, useWaitlistStats, useApproveWaitlistEntry, useRejectWaitlistEntry, useBulkApproveWaitlist } from '../../hooks/useAdminApi'
import { Check, X, ChevronLeft, ChevronRight, Clock, CheckCircle, XCircle, Loader2 } from 'lucide-react'
...
export function AdminWaitlistPage() {
  ...
  const { data, isLoading } = useAdminWaitlist({ page, limit: 20, status: statusFilter })
  ...
  const handleBulkApprove = async () => {
    ...
    result.results?.forEach((r: { id: string; code: string }) => { codes[r.id] = r.code })
  ...
  const toggleSelectAll = () => {
    ...
    const pendingIds = data.items.filter(e => e.status === 'pending').map(e => e.id)
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">{t('waitlist.title')}</h1>

        {/* Stats */}
        {stats && (
          <div className="grid grid-cols-4 gap-4 mb-6">
            {(['pending', 'approved', 'rejected', 'total'] as const).map(key => (
              <div key={key} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                <div className="text-sm text-gray-500 dark:text-gray-400">{t(`waitlist.stats.${key}`)}</div>
        ...
        <div className="flex gap-2 mb-4">
          {(['pending', 'approved', 'rejected'] as const).map(status => (
            ...
            >
              {t(`waitlist.${status}`)}
              {stats && (
                <span className="ml-1.5 text-xs opacity-75">({stats[status]})</span>
              )}
            </button>
          ))}
        </div>
                ...
                <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                  {statusFilter === 'pending' && (
                    <th className="px-4 py-3 text-left">
                      <input
                        type="checkbox"
                        checked={items.filter(e => e.status === 'pending').length > 0 && selectedIds.size === items.filter(e => e.status === 'pending').length}
                        onChange={toggleSelectAll}
                        className="rounded border-gray-300 dark:border-gray-600"
                      />
                    </th>
                  )}
                  <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-400">{t('waitlist.columns.email')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-400">{t('waitlist.columns.name')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-400">{t('waitlist.columns.submitted')}</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-400">{t('waitlist.columns.status')}</th>
                  <th className="px-4 py-3 text-right font-medium text-gray-600 dark:text-gray-400">{t('waitlist.columns.actions')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {items.map(entry => (
                  <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                    ...
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100 font-mono text-xs">{entry.email}</td>
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{entry.name || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      {approvedCodes[entry.id] && (
                        <span className="text-xs text-green-600 dark:text-green-400 font-mono">
                          {t('waitlist.codeSent', { code: approvedCodes[entry.id] })}
                        </span>
                      )}
                      {entry.status === 'approved' && entry.emailSentAt && !approvedCodes[entry.id] && (
                        <span className="text-xs text-gray-400">{t('waitlist.emailSent')}</span>
                      )}
