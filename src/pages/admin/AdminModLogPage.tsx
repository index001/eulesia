...
export function AdminModLogPage() {
  ...
  return (
    <AdminLayout>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">{t('modlog.title')}</h1>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{t('modlog.action')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{t('modlog.target')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{t('modlog.admin')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{t('modlog.reason')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{t('modlog.date')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {data?.items?.map(entry => (
                  <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{entry.adminName}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 max-w-xs truncate">{entry.reason}</td>
                    <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{formatRelativeTime(entry.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {!data?.items?.length && (
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">{t('modlog.noEntries')}</div>
            )}
          </div>
...
function ActionBadge({ type }: { type: string }) {
  const colors: Record<string, string> = {
    ...
    report_dismissed: 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200',
    report_resolved: 'bg-blue-100 text-blue-800',
    role_changed: 'bg-purple-100 text-purple-800'
  }
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${colors[type] || 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200'}`}>
      {type.replace(/_/g, ' ')}
    </span>
  )
}
